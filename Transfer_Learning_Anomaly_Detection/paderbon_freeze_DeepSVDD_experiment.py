...
import paderbon_freeze_DeepSVDD as DS
import copy
...
def Train_deepsvdd(args, device):
    if __name__ == '__main__':
        ...
        # 미리 학습된 가중치 파일 경로
        pretrained_weight_path = "/Transfer_Learning_Anomaly_Detection\\best_acc_parameters.pth"

        # 미리 학습된 가중치를 로드하여 모델에 적용
        pretrained_weights = torch.load(pretrained_weight_path)

        # 모델 생성
        model = deep_SVDD.model
        ...
        if args.pretrain:
            deep_SVDD.pretrain(model)

        # 학습된 가중치로 Deep_SVDD모델 Train
        net, c = deep_SVDD.train(model)
...
def Experiment(args,device, repeat):
    ...
    #                 'net_dict': best_model_weight},'C://Users//gyuhee//PycharmProjects//pythonProject//model_save//best_acc_parameters.pth')
