...
def Experiment(args,device, repeat):
    ...
    Result = []
    worst_auc = float('inf')
    worst_model_weight = None  # 초기 최저 모델의 가중치
    worst_c = None
    ...
    for i in range(repeat):
        ...
        if roc_auc_value < worst_auc:
            worst_auc = roc_auc_value
            worst_model_weight = copy.deepcopy(deepsvdd_net.state_dict())
            worst_c = copy.deepcopy(deepsvdd_c)
    ...
    if worst_model_weight is not None and worst_c is not None:
        torch.save({'center': worst_c.cpu().data.numpy().tolist(),
                    'net_dict': worst_model_weight},
                   '/Transfer_Learning_Anomaly_Detection/worst_retrained_parameters_Testrig300.pth')
