...
class NPZLoader(data.Dataset): #넘파이 파일을 불러오는 기능
    ...
    def __getitem__(self, index): #각 인덱스에 해당하는 데이터를 불러옴
        x = torch.from_numpy(np.load(self.data[index])['x'])
        x = x.unsqueeze(0)
...
# def get_data(args, data_dir = r'C:\Users\gyuhee\Desktop\연구실\Bearing_Data\TEST_RIG_데이터변형\2D_npz\6207_npz'):
def get_data(args, data_dir=r"C:\Users\user\Desktop\연구실\spectrogram\스펙트로그램_class7\6207\2024\900"):
    """get dataloders"""
    ...
    for class_name in classes:
        ...
        if class_name == normal_class:
            # npz_list_train = npz_list[:3000]
            # npz_list_test = npz_list[3000:4000]
            npz_list_train = npz_list[:25]
            npz_list_test = npz_list[25:125]
            ...
            # npz_list_test = npz_list[:1000]
            npz_list_test = npz_list[:100]
            # print(f"loaded {len(npz_list_test)} files from {class_folder} for testing")
            data_test.extend(npz_list_test)
...
class TrainerDeepSVDD:
    ...
    def train(self):
        ...
        if self.args.pretrain == True:
            state_dict = torch.load('C:/Users/user/Desktop/python/Transfer_Learning_Anomaly_Detection/retrained_parameters_필요없음.pth')
